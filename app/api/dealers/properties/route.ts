...
export async function POST(request: Request) {
  ...
  const property = await createVendorPropertyInStore(session.dealerId, {
    ...
    sourceType: "VENDOR",
    sourcePlatform: parsed.data.sourcePlatform || "DEALER_DIRECT",
    sourceUrl: parsed.data.sourceUrl || null,
    priceLastVerified: parsed.data.priceLastVerified || null,
    photoRightsStatus: parsed.data.photoRightsStatus || "DEALER_UPLOADED",
    approvalStatus: "PENDING",
    boostTier: parsed.data.boostTier || "STANDARD",
